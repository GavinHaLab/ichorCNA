...
normal <- parse_numeric_vector(opt$normal)
normal.init <- parse_numeric_vector(opt$normal.init)
scStates <- parse_numeric_vector(opt$scStates)
subclone.penalty <- opt$scPenalty
likModel <- opt$likModel
lambda <- parse_numeric_vector(opt$lambda)
...
ploidy <- parse_numeric_vector(opt$ploidy)
...
plotYLim <- parse_numeric_vector(opt$plotYLim)
outImage <- paste0(outDir,"/", patientID,".RData")
genomeBuild <- opt$genomeBuild
genomeStyle <- opt$genomeStyle
chrs <- as.character(parse_numeric_vector(opt$chrs))
chrTrain <- as.character(parse_numeric_vector(opt$chrTrain))
chrNormalize <- as.character(parse_numeric_vector(opt$chrNormalize))
...
.repTime_supported <- "repTime" %in% names(formals(loadReadCountsFromWig))
.gDP_formals   <- names(formals(getDefaultParameters))
.hmm_formals   <- names(formals(HMMsegment))
.cICN_formals  <- names(formals(correctIntegerCN))
...
if (!is.null(repTimeWig) && nchar(repTimeWig) > 0) {
...
for (i in 1:numSamples) {
  ...
  counts[[id]] <- do.call(loadReadCountsFromWig, c(
    list(tumour_reads, chrs = chrs, gc = gc, map = map,
    ...
    if (.repTime_supported && !is.null(repTime)) list(repTime = repTime) else list()
  ))
  ...
  if (!is.null(normal_file) && normal_file != "None" && normal_file != "NULL"){
    ...
    counts.normal <- do.call(loadReadCountsFromWig, c(
      list(normal_reads, chrs=chrs, gc=gc, map=map,
...
for (i in 1:length(ploidy)){
  ...
  for (j in 1:nrow(normal.restarts)){
    ...
    .gDP_args <- list(
      maxCN = maxCN, includeHOMD = includeHOMD, ct.sc = scStates,
      e = txnE, e.sameState = multSampleTxnStrength, strength = txnStrength,
      n_0 = n, normal2IgnoreSC = normal2IgnoreSC, ploidy_0 = floor(p),
      e.subclone = subclone.penalty, likModel = likModel
    )
    param <- do.call(getDefaultParameters, c(
      list(logR[valid & chrInd, , drop=F]),
      .gDP_args[names(.gDP_args) %in% .gDP_formals]
    ))
    ...
    #############################################
    .hmm_named <- list(
      dataType = "copy", param = param, chrTrain = chrTrain, maxiter = 20,
    ...
    hmmResults.cor <- do.call(HMMsegment, c(
      list(tumour_copy, valid),
      .hmm_named[names(.hmm_named) %in% .hmm_formals]
    ))
    ...
    for (s in 1:numSamples){
      ...
      .cICN_args <- list(
        cn = hmmResults.cor$cna[[s]], segs = hmmResults.cor$results$segs[[s]],
        purity = 1 - hmmResults.cor$results$n[s, iter],
        ploidy = hmmResults.cor$results$phi[s, iter],
        cellPrev = 1 - hmmResults.cor$results$sp[s, iter],
        maxCNtoCorrect.autosomes = maxCN, maxCNtoCorrect.X = maxCN,
        minPurityToCorrect = 0.05, gender = gender$gender,
        chrs = chrs, correctHOMD = includeHOMD
      )
      correctedResults <- do.call(correctIntegerCN, .cICN_args[names(.cICN_args) %in% .cICN_formals])
...
for (i in 1:length(ind)) {
  ...
  plotSolutions(hmmResults.cor, tumour_copy, chrs, temp_outDir, counts,
                numSamples = numSamples, logR.column = "logR", call.column = "event",
                likModel = likModel, plotFileType = plotFileType, plotYLim = plotYLim,
                seqinfo = seqinfo, estimateScPrevalence = estimateScPrevalence, maxCN = maxCN)
}
